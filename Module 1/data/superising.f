...
CC=====================================================
        ...
        open(4,file='L50picco.txt',status='unknown') 
...
CC====================================== 
        ...
        read(1,*) beta           !!valore di 1/(kT) = beta (è il valore iniziale)

        do inn = 1,20
            call ranstart   

            open(2,file='data',status='unknown', blank='null')
            open(3,file='chi.txt',status='unknown',blank='null')

            beta = beta + 1/float(npasso)
...
CC   MISURA DELLE OSSERVABILI FISICHE
	      ...
	      enddo 
            close(2)
            xavm = xavm/float(measures)
            xavc = xavc/float(measures)
	 	...
	 	enddo
            sigm = sigm/(float(measures)*(float(measures) - 1))
            sigc = sigc/(float(measures)*(float(measures) - 1))
