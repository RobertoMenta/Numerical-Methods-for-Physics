...
c*****************************************************************
      ...
      do i = 1,nlatt2            !! loop su tutti i siti, qui il sito 
         ...
         if (x.lt.p_rat) field(i) = phi_prova !! p_rat > 1 -> se si accetto FUNZIONA. VERIFICATO
c         write(*,*) field(i)          
	  ...
	  do ii=1,nlatt
		 ...
		 s=abs(field(k) - field(k + nlatt))
		 t=abs(field(ii) - field(ii + nlatt))
         if (t.le.s) k=ii       !! Funziona correttamente
...
c*****************************************************************
      ...
      do iu =1,measures
	 	sig = sig + (q(iu) - xav)**2
	enddo
            sig = sig/(float(measures)*(float(measures) - 1))
      ...
      do ku=1,5000
		do ju=1,measures-ku
		   con = con + (q(ju) - xav)*(q(ju+ku) - xav)
        enddo
		con = con * (1/(float(measures)-ku))
        xtau = xtau + con
		con = 0.0
	  enddo
      var = sig*(1+2*xtau)
