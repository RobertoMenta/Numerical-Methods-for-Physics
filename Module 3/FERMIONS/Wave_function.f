	  ...
	  parameter (nx=400)
      real lattice,sigm,xr(1000000),xs(1000000),t(1000000),u(400),y(400)
	  real r(1000000),s(1000000),xpara
      integer seed
      ...
      common/sig/sigma
      common/segno/sigm
	  common/matrix/xq(nx,nx)
	  
	  
      open(1,file='input.txt',status='old')
      open(2,file='meas_out.txt',status='unknown')
	  open(3,file ='data.txt',status='unknown')
      open(7,file='camp.txt',status='unknown')
	  open(9,file='datifunzione.txt',status='unknown')
CC====================================
...
CC   AGGIORNAMENTO CONFIGURAZIONE
	  ...
	  sigm=sigm/float(measures*i_decorrel)
	  
	  close(7)
       
	  open(7,file='camp.txt',status='old')
	  
	  do ij= 1,1000000
	    read(7,*) xr(ij),xs(ij),t(ij)
	  enddo
	  write(*,*) sigm
	  do ix=1,400
	    do it=1,400
	    ...
	    do ih =1,400
		  do ip=1,400
		    do jk =1,1000000
	            r(jk)=abs(xr(jk) - u(ih))
				s(jk)=abs(xs(jk) - y(ip))
				if(r(jk).lt.xpara .and. s(jk).lt.xpara ) then  
					   if(t(jk).eq.1) then
					   xq(ih,ip) = xq(ih,ip) + 1.0/sigm
                       else 
					   xq(ih,ip) = xq(ih,ip) - 1.0/sigm
					   endif
			    endif
...
C      open(7,file='path.txt', status='unknown')
      do i = 1,nlatt2          
         ...
         if (x.lt.p_rat) field(i) = phi_prova 
c         write(7,*) i,field(i)
      enddo                     
         ...
         do i= 1,nlatt
		   if(sigma.eq.0) then	  
            write(7,*) field(i),field(nlatt +i), 1
			sigm= sigm + (1/float(nlatt))
