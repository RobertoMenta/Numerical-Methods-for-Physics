...
C--------------------------------------------------------------------
      parameter(nx = 100,nt = 2 ,nvol = nx*nt)
...
c=================================================================
      parameter(nx = 100, nt = 10,nvol = nx*nt)
      parameter(nlatt = nx)   !! must be the maximum dimension size
      parameter(pigr = 3.141592654)
      real mass,extfield,mass2,mass2p4  
      common/various/mass,extfield,mass2,mass2p4 !! parametri della simulazione
...
c	nfake= 1000
	...
	open(2,file='data.txt',status='unknown')
	open(5,file ='data_tot.txt',status='unknown')
	do i=1,measures 
		read(2,*) r(i),s(i),t(i)
	enddo
      do j =1,measures
	...
	enddo
      ...
      close(2)
	close(5)
      open(5, file='data_tot.txt',status = 'old')
	...
	xmed = xmed/float(nfake)
      x1 = x1/(float(nfake))
	var= sqrt(x1 - (xmed)**2)
      var = var*nt**2/float(2)
	xmed = (xmed - 0.00343003217)*nt**2/float(2)
